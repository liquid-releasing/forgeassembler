...
- [ ] **Help menu in the title bar** — native pywebview menu with Documentation (link to mkdocs site), Discord (community link), and About (link to repo or in-app dialog). Each menu item opens in the user's default browser via `webbrowser.open`. macOS: full Cocoa menu bar support. Windows + Linux: backend-dependent — wrap in try/except so unsupported platforms degrade gracefully (no menu, no crash).
